...
export default function EmpDashboard({ setPage }) {
  ...
  const statusMap = {
    ...
    holiday:  { label: "Holiday",        color: T.primary, msg: "Enjoy your holiday today! Office is closed."      },
    weekend:  { label: "Weekend",        color: T.muted,   msg: "It's the weekend — enjoy your time off!"          },
    future:   { label: "Upcoming",       color: T.muted,   msg: "This is a future date."                           },
  };
  // Always fall back to absent so tod is never undefined
  const tod = statusMap[status] || statusMap["absent"];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{
        ...
        <div style={{ position: "relative" }}>
          ...
          <div style={{ fontWeight: 800, fontSize: 26, color: "#fff", marginBottom: 6 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16 }}>
        {[
          { label: "Attendance Rate",  value: `${attRate}%`, svgKey: "trending", color: T.primary,  bg: T.primaryLight, sub: "This month" },
          { label: "Days Present",     value: present,        svgKey: "check",    color: T.success,  bg: "#d1fae5",      sub: "March 2026" },
          { label: "Leaves Remaining", value: [12,10,15,4].reduce((a,b)=>a+b,0) - (employee?.id ? leaves.filter(l=>l.employee_id===employee.id&&l.status==="approved").reduce((a,l)=>a+(l.days||0),0) : 0),
                                                              svgKey: "calendar", color: T.accent,   bg: "#dbeafe",      sub: "All types" },
          { label: "Pending Tasks",    value: tasks.filter(t=>!t.done).length,
                                                              svgKey: "clipboard", color: T.warning,  bg: "#fef3c7",      sub: "Action needed" },
        ].map(s => (
          <Card key={s.label} style={{ padding: "18px 20px" }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
              <div>
                ...
                <div style={{ fontSize: 30, fontWeight: 800, color: T.text, lineHeight: 1 }}>{s.value}</div>
              ...
              <div style={{ width: 48, height: 48, borderRadius: 14, background: s.bg, color: s.color, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                {s.svgKey === "trending"   && <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>}
                {s.svgKey === "check"      && <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="m9 12 2 2 4-4"/></svg>}
                {s.svgKey === "calendar"   && <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>}
                {s.svgKey === "clipboard"  && <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="8" y="2" width="8" height="4" rx="1"/><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><path d="M12 11h4"/><path d="M12 16h4"/><path d="M8 11h.01"/><path d="M8 16h.01"/></svg>}
              </div>
            </div>
          </Card>
        ))}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        ...
        <Card style={{ padding: 20 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {[
              { type: "Casual Leave",  total: 12, used: employee?.id ? leaves.filter(l=>l.employee_id===employee.id&&l.leave_type==="Casual Leave"&&l.status==="approved").reduce((a,l)=>a+(l.days||0),0) : 0, color: "#0061f2" },
        ...
        <Card style={{ padding: 20 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {(announcements.length ? announcements : []).slice(0,3).map(a => {
              const tag = a.tag || "HR";
              const color = { HR:"#0061f2", Holiday:"#10b981", Policy:"#f59e0b", Wellness:"#06b6d4", IT:"#8b5cf6", Event:"#ec4899" }[tag] || "#0061f2";
              return (
              <div key={a.id} style={{
        ...
        >
          ...
          >
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <span style={{ fontSize:18, fontWeight:800, color:T.text }}>Add New Task</span>
